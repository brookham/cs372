...
def dijkstras_shortest_path(routers, src_ip, dest_ip):
    ...
    **Strong recommendation**: make functions to do subtasks within this
    ...
    src = netfuncs.find_router_for_ip(routers, src_ip)
    dst = netfuncs.find_router_for_ip(routers, dest_ip)
    ...
    while to_visit:
        shortest = min(to_visit, key=distance.get)

        to_visit.remove(shortest)

        for neighbor in routers[shortest]["connections"]:
            if neighbor in to_visit:
                alt = distance[shortest] + routers[shortest]["connections"][neighbor]["ad"]
                if alt < distance[neighbor]:
                    distance[neighbor] = alt
                    parent[neighbor] = shortest

    
    current = dst

    while current is not None and current != src:
        path.append(current)
